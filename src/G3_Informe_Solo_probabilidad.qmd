...
**Espacio muestral:** Son los niveles de riqueza que se encuentran en la variable *indice_riqueza* de la tabla RECH23.
...
El nivel de riqueza que es más probable de encontrar en al escoger un hogar es muy pobre, mientras que el menos probable de ser escogido es que sea muy rico.
...
**Espacio muestral:** Son la etiquetas dadas a cada infante según su nivel de anemia. Se encuentran en la variable *anemia* de la tabla REC44.
...
Probabilidad que ocurra A1 y A2, obtenida de forma manual

```{r}
'A1_&_A2' <- choose(36368,1)*choose(971,1)/choose(36368,2)
`A1_&_A2`
```

Y vemos la probabilidad de que A1 ocurra dado A2

$$
P(A1 \mid A2) = \frac{P(A1 \cap A2)}{P(A2)}
$$

```{r}
A1_dado_A2 <- `A1_&_A2`/A2
A1_dado_A2
```

Al hacer que un evento ocurra dado otro, este da un resultado, por lo que uno si afecta al otro. Eso se debe a que el experimento toma un valor que no se va a reponer, o sea, por cada evento que siga, disminuye el tamaño de la muestra.
