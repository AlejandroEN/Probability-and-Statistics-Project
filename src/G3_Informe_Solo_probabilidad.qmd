...
Para esta parte vamos a usar de nuestros conocimientos sobre probabilidades para trabajar con eventos que seleccionamos

### Probabilidad empírica

Primero seleccionamos dos experimentos, con su espacio muestral y sus respectivas probabilidades.

**Experimento 1.** Escoger un hogar que aceptó participar en la encuesta y ver la región a la que pertenece.

**Espacio muestral:** Son los nombres de cada región del Perú, que se encuentran en la variable *región* de la tabla REC0111.

```{r}
unique(REC0111$region)
```

**Probabilidad de cada evento:**

```{r}
prob_region <- table(REC0111$region)/sum(table(REC0111$region))
prob_region
```

```{r}
sum(table(REC0111$region)/sum(table(REC0111$region)))
```

Región con mayor y menor probabilidad de ser escogida.

```{r}
names(which.max(prob_region))
names(which.min(prob_region))
```

La región que tiene mayor probabilidad de ser escogida es Lima (razonable porque es la región con mayor población), y quien tiene menor probabilidad es Puno.
...
**Espacio muestral:** Son la etiquetas dadas a cada infante participante según su nivel de anemia. Se encuentran en la variable *anemia* de su tabla (REC44).
...
Por los resultados, es mucho mas probable que de entre los niños escogidos, este no tenga anemia. Y es menos probable que se encuentre en un estado grave de anemia.

## Probabilidad condicional

Ahora vamos a escoger dos pares de eventos. Uno sera independiente y otro no independiente.
...
```{r}
'A1_&_A2' <- nrow(filter(RECH5, is_embarazada == 0 & is_embarazada == 1))/nrow(RECH5)
`A1_&_A2`
```

La intersección de estos dos eventos resultó ser 0 porque los dos son mutuamente excluyentes, o sea, se puede escoger a uno pero no al otro. Y por propiedad, al ser mutuamente excluyentes, no son independientes.

Dado que al ser no independientes no son disjuntos, podemos hacer las siguientes comprobaciones.

Al ser ambos mutuamente excluyentes y corresponder a todo el espacio muestral de un experimento, estamos ante una partición, donde se puede comprobar el teorema de la probabilidad total.

Teorema de la probabilidad total.

$$ P(B) = P(B \mid A1)*P(A1) + P(B \mid A2)*P(A2) $$

Para eso, vamos a tomar un evento que pueda ocurrir en ambos. Este será el evento B, que representará si la mujer seleccionada tiene como mayor nivel educativo la secundaria.
...
prob_total_b = prob_b_dado_a1 + prob_b_dado_a2
...
Con esto, se puede concluir que la probabilidad de que la mujer escogida resulte embarazada sabiendo que esta embarazada es 0.02863548
...
Se puede concluir que la probabilidad de que la mujer escogida resulte embarazada sabiendo que no esta embarazada es 0.9713645
...
Para comprobar que son independientes, vamos a comparar la intersección de eventos de forma manual con el producto de ambas probabilidades. Según propiedad, para dos eventos independientes, el producto de sus probabilidades resulta ser igual a la intersección.
